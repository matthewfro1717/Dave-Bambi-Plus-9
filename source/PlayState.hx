				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent)
				{
					...
					switch (daNote.noteStyle)
					{
						...
						default:
							...
							if (daNote.noteStyle == 'GF Sing') { // I didn't notice bambi's alt animation has only left and right
								if (noteToPlay == 'RIGHT') noteToPlay = 'RIGHT';
								if (noteToPlay == 'LEFT') noteToPlay = 'LEFT';
								if (noteToPlay == 'UP') noteToPlay = 'UP';
								if (noteToPlay == 'DOWN') noteToPlay = 'DOWN';
							}
